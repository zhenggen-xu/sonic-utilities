...
class TestConfigMgmt(TestCase):
    ...
    def config_mgmt_dpb(self, curConfig):
        ...
        cmdpb.verifyAsicDB = MagicMock(return_value=True)
        cmdpb._shutdownIntf = MagicMock(return_value=True)
    ...
    def generate_args(self, portIdx, laneIdx, curMode, newMode):
        ...
        Return:
            dPorts, aPorts, pJson (tuple)[list, dict]
        '''
        # default params
        pre = "Ethernet"
    ...
    def dpb_port8_4x25G_2x50G_f_l(self, curConfig):
        ...
        cmdpb.breakOutPort(delPorts=dPorts, addPorts=aPorts, portJson=pJson, force=True, \
            loadDefConfig=True)
        # Expected Result delConfig and addConfig is pushed in order
        delConfig = {
            u'ACL_TABLE': {
